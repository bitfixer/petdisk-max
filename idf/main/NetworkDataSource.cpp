...
bool NetworkDataSource::openFileForReading(unsigned char* fileName)
{
    ...
    if (bytesToFetch > _fileSize) {
        bytesToFetch = _fileSize;
    }

    fetchBlock(0, bytesToFetch);
...
void NetworkDataSource::updateBlock()
{
    ...
    struct receiveBuffer* recvBuffer = (struct receiveBuffer*)_dataBuffer;
    char tmp[128];
    sprintf_P(tmp, PSTR("?f=%s&u=1&s=%" PRIu32 "&e=%" PRIu32), recvBuffer->fileName, _currentOutputByte, endByte);
    strcpy(info->params, tmp);
