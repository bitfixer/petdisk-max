 ...
 */
void http_native_request(void* args)
{
    //bool* done = (bool*)args;
    ESP_LOGI(TAG, "http_native_request");
    // Declare local_response_buffer with size (MAX_HTTP_OUTPUT_BUFFER + 1) to prevent out of bound access when
    // it is used by functions like strlen(). The buffer should only be used upto size MAX_HTTP_OUTPUT_BUFFER
    char output_buffer[1024];   // Buffer to store response of http request
    int content_length = 0;
    /*
    esp_http_client_config_t config = {
        .url = "http://192.168.0.168/petdisk.php?file=TEST.PRG&s=0&e=512",
        .port = 8080
    };
    */
    esp_http_client_config_t config;
    memset(&config, 0, sizeof(esp_http_client_config_t));
    config.url = "http://192.168.0.168/petdisk.php?file=TEST.PRG&s=0&e=512";
    config.port = 8080;
    esp_http_client_handle_t client = esp_http_client_init(&config);
